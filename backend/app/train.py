...
vn.train(ddl="""CREATE TABLE `discipline_data` (
  ...
  `date` datetime DEFAULT NULL COMMENT 'Exact initiation datetime (yyyy-mm-dd 00:00:00)',
  ...
  `ofr_rank` varchar(64) DEFAULT NULL COMMENT 'Rank of the person',
  ...
  `year` int DEFAULT NULL COMMENT 'Year when the case was initiated (e.g., 2020)',
  `svc_bracket` int DEFAULT NULL COMMENT 'Service in years at time of award (e.g., 5)',
  `unit` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT NULL COMMENT 'Unit where the person received the award',
  PRIMARY KEY (`id`),
  KEY `ix_discipline_data_name` (`name`),
  KEY `ix_discipline_data_svc_no` (`svc_no`),
  KEY `ix_discipline_data_initiation_year` (`year`),
  KEY `ix_discipline_data_course` (`course`)
) ENGINE=InnoDB AUTO_INCREMENT=29 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci""")

vn.train(question="Find all people who were awarded in the year 2020.", sql="""SELECT name, ofr_rank, award 
FROM discipline_data
WHERE year = 2020;
""")
vn.train(question="List all distinct categories of offences.", sql="""SELECT DISTINCT cat 
FROM discipline_data;
""")
vn.train(question="Get the names and ranks of people from unit 'Alpha Unit'.", sql="""SELECT name, ofr_rank 
FROM discipline_data
WHERE unit = 'Alpha Unit';
""")
# vn.train(question="df", sql="""""")
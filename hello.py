...
vn.train(
    question="Show me all expenses for a specific user this month",
    sql="SELECT e.name, e.amount, e.date, sh.heads as sub_head FROM expenses e JOIN sub_heads sh ON e.sub_head_id = sh.id WHERE e.user_id = ? AND MONTH(e.date) = MONTH(CURRENT_DATE()) AND YEAR(e.date) = YEAR(CURRENT_DATE())"
)
...
print("")
print("📈 Expected Accuracy: ~80% (with contextual examples)")
print("🔧 Ready for queries! Use: vn.ask('Your question here')")

# Test queries to validate training effectiveness
print("\n🧪 Testing AI SQL Agent:")
print("You can now test with queries like:")
print("- 'How many active users are there?'")
print("- 'Show me the current balance for each formation'")
print("- 'What are all unpaid liabilities?'")
print("- 'Who are the current commanding officers?'")
print("- 'Show me the chain of command hierarchy'")
print("\nExample usage:")
print("result = vn.ask('How many active users are there in the system?')")
print("print(result)")